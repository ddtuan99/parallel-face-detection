...
def detect_kernel(base_win_sz, stage_thresholds, tree_counts, 
                  ...
                  scale, scale_idx, sld_w, sld_h, step, num_candidates, is_pass, 
                  start_stage, end_stage, write_idx, n_live_cands):
    img_width = sat.shape[1] - 1
    if start_stage == 0:
        i, j = cuda.grid(2)
        x, y = i * step, j * step
        id = x + y * img_width
    else:
        if cuda.grid(1) >= n_live_cands:
            return;
        id = is_pass[num_candidates[scale_idx] + cuda.grid(1)]
        x, y = id % img_width, id // img_width
    x_sat = x
    y_sat = y
    ...
    if x_sat < sld_w and y_sat < sld_h:
        ...
        for stg_idx in range(start_stage, end_stage):
            stg_sum = 0.0
        ...
        idx = cuda.atomic.add(write_idx, 0, 1)
        is_pass[num_candidates[scale_idx] + idx] = id
...
def detect_multi_scale(model, sats, out_img, scale_factor=1.1):
    ...
    while scale < max_scale:
        scale_data.append([scale, cuda.stream()])
    ...
    d_is_pass = cuda.device_array(total_candidates, dtype=np.int32)
    ...
    # Invoke first segment kernel of each scale
    for scale_idx, sc_data in enumerate(scale_data):
        scale = sc_data[0]
        stream = sc_data[1]
        ...
        d_write_idx = cuda.to_device(np.zeros(1, dtype=np.int32), stream)

        event = cuda.event(timing=False)

        sc_data.append(d_rectangles)
        sc_data.append(d_weights)
        sc_data.append(d_write_idx)
        sc_data.append(event)
        sc_data.append(1)
        ...
        start_stg, end_stg = stage_segments[:2] # first segment
        ...
        detect_kernel[grid_size, block_size, stream](d_base_win_size, d_stage_thresholds, d_tree_counts, 
                                                ...
                                                sld_w, sld_h, step, d_num_candidates, d_is_pass, 
                                                start_stg, end_stg, d_write_idx, np.int32(-1))
        event.record(stream)

    num_scales = len(scale_data)
    num_segments = len(stage_segments) - 1
    num_kernels = num_scales * (num_segments - 1) # Number of kernel need to be invoke

    # Polling
    while num_kernels > 0:
        for sc_idx, sc_data in enumerate(scale_data):
            event, segment = sc_data[5], sc_data[6]

            if segment + 1 > num_segments or not event.query():
                continue
            
            scale, stream, d_rectangles, d_weights, d_write_idx = sc_data[:5]

            n_live_cands = d_write_idx.copy_to_host(stream=stream)[0]
            if n_live_cands == 0:
                sc_data[6] = num_segments
                num_kernels -= num_segments - segment
                continue
            d_write_idx = cuda.to_device(np.zeros(1, dtype=np.int32), stream)
            sc_data[4] = d_write_idx
            
            start_stg, end_stg = stage_segments[segment], stage_segments[segment + 1]
            ...
            block_size = 32
            grid_size = math.ceil(n_live_cands / block_size)
            
            detect_kernel[grid_size, block_size, stream](d_base_win_size, d_stage_thresholds, d_tree_counts, 
                                        d_feature_vals, d_rect_count, d_rectangles, d_weights, 
                                        sats[0], sats[1], scale, sc_idx, 
                                        sld_w, sld_h, step, d_num_candidates, d_is_pass, 
                                        start_stg, end_stg, d_write_idx, n_live_cands)
            event.record(stream)
            sc_data[6] += 1
            num_kernels -= 1
    cuda.synchronize()
    # e_event = cuda.event()
    # e_event.record(cuda.default_stream())
    # print(cuda.event_elapsed_time(s_event, e_event))

    is_pass = d_is_pass.copy_to_host(stream=cuda.stream())

    tmp_stream = cuda.stream()
    live_cands = np.empty(num_scales, dtype=np.int32)
...
@jit(nopython=True, cache=True)
def delete_zero(win_size, height, width, scale_factor, is_pass, live_cands, num_candidates):
    scale = 1.0
    max_scale = min(width / win_size[0], height / win_size[1])    
    rec_list = List()
    scale_idx = 0
    while scale < max_scale:
        cur_win_size = (win_size * scale).astype(np.int32)
        for i in range(num_candidates[scale_idx], num_candidates[scale_idx] + live_cands[scale_idx]):
            x, y = is_pass[i] % width, is_pass[i] // width
